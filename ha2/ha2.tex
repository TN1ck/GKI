...
\begin{center}
    \begin{tikzpicture}[auto,bend angle=30,node distance=3cm]
        \node[state, circle] (S) {};
        \node[state, circle] (1)[below left of=S] {1};
        \node[state, circle] (2)[below right of=S] {2};

        \path[->, bend right] (S) edge node {bewegen(c)} (1);
        \path[->, bend left] (S) edge node {bleiben} (2);

        \node[state, circle] (3)[below left of=1] {3};
        \node[state, circle] (4)[below right of=1] {4};
        \node[state, circle] (5)[below right of=2] {5};

        \path[->, bend right] (1) edge node {bleiben} (3);
        \path[->, bend left] (1) edge node {bewegen(a)} (4);
        \path[->, bend left] (2) edge node {bleiben} (5);

        \node[state, circle] (6)[below left of=3] {6};
        \node[state, circle] (7)[below right of=3] {7};
        \node[state, circle] (8)[below of=4, node distance=2.12132cm] {8};
        \node[state, circle] (9)[below left of=5] {9};
        \node[state, circle] (10)[below right of=5] {10};

        \path[->, bend right] (3) edge node {bleiben} (6);
        \path[->, bend left] (3) edge node {bewegen(a)} (7);
        \path[->, bend left] (4) edge node {bleiben} (8);
        \path[->, bend right] (5) edge node {bewegen(c)} (9);
        \path[->, bend left] (5) edge node {bleiben} (10);
    \end{tikzpicture}
\end{center}
...
\item
Eine Idee für eine Bewertungsfunktion ist die jeweiligen Knoten zu betrachten die man vor dem Gegner erreichen kann.	
Die Bewertung eines Zustandes im Spiel kann man durch den folgenden Algorithmus bekommen:
Sei $n=0$, färbe alle Knoten weiß und habe jeder Roboter ein Konto mit seinen aktuellen Ressourcen.
\begin{enumerate}[1)]
\item
Man betrachte alle Knoten die der Roboter in n Schritten erreichen kann, der als nächstes am Zug ist. Hierbei muss man in Betracht ziehen, dass manche Knoten nicht erreichbar sind, da der andere Roboter den Weg blockiert
\begin{enumerate}[~~-]
\item
Bei einem weißen Knoten addiert der Roboter die Ressourcen des Knoten minus den Wegkosten zu diesen Knoten auf sein Konto.
\item 
Bei einen Knoten der auf zwei Wegen erreichbar ist nimmt man den kostengünstigeren Weg. 
\item
 Graue Knoten ignoriert man.
\end{enumerate}
\item 
Anschließend färbt man alle gerade besuchten Knoten grau.
\item 
Nun werden die ersten beiden Schritte mit dem anderen Roboter wiederholt.
\item
Erhöhe n um 1 und führe Schritte 1 bis 4 durch, bis alle Knoten grau sind.
\item
Das Ergebnis ist die Differenz der beiden Konten der Roboter.
\end{enumerate}

Diese Bewertung zieht halt nicht nur in Betracht wie viele Ressourcen die Roboter bereits gesammelt haben, sondern auch wie viel sie jeweils potentiell noch sammeln könnten. Somit würde auch im obigen Beispiel der erste Roboter sich nicht gleich auf c stürzen um dort schnell Ressourcen zu sammeln sondern auf a bleiben um den 2. Roboter zu blockieren.
